...
public class GridTest {
    ...
    public void should_get_first_row_when_one_row() throws Exception {
        // Given
        Grid grid = new Grid(GridHintsBuilder.gridHints().withLineHint(0, 0, 3).withLineHint(0, 8, 9).build(), 9);

        // When
        LineHints lineHints = grid.getRow(0);

        // Then
        assertThat(lineHints).isEqualTo(LineHintsBuilder.lineHints().withLineHint(0, 3).withLineHint(8, 9).build());
    }

    @Test
    public void should_get_second_row_when_two_rows() throws Exception {
        // Given
        Grid grid = new Grid(GridHintsBuilder.gridHints().withLineHint(1, 0, 3).withLineHint(0, 8, 9).build(), 9);

        // When
        LineHints lineHints = grid.getRow(1);

        // Then
        assertThat(lineHints).isEqualTo(LineHintsBuilder.lineHints().withLineHint(0, 3).build());
    }
}